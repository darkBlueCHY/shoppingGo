...
public class Item implements Serializable {
    ...
    public Item transfrom(TbItem item) {
        this.id = item.getId();
        ...
        this.costPirce = item.getCostPirce();
        this.marketPrice = item.getMarketPrice();
        this.isDefault = item.getIsDefault();
        this.goodsId = item.getGoodsId();
        this.sellerId = item.getSellerId();
        this.cartThumbnail = item.getCartThumbnail();
        this.categoryid = item.getCategoryid();
        this.brand = item.getBrand();
        this.spec = item.getSpec();
        Map<String,String> tmp = JSONObject.parseObject(this.spec, Map.class);
        try {
            this.specMap = new HashMap();
            if (tmp != null) {
                for (String key : tmp.keySet()) {
                    String postfix = new String(Base64.getEncoder().encode(DigestUtils.md5Digest(key.getBytes())));
                    this.specMap.put(postfix.substring(0,postfix.length()-2),tmp.get(key));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
